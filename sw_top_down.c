...
int top_down_sw(char* s1, char* s2, int i, int j) {
    ...
    if (d > res) res = d;
    if (u > res) res = u;
    if (l > res) res = l; // Fixed: using 'l' to match the variable above
    
    return memo[i][j] = res;
}

int main() {
    // Using your specific 10bp vs 9bp sequences
    char *s1 = "GACCTACAAG";
    char *s2 = "GACGTAGAA";
    ...
    // Search the entire matrix for the maximum score
    for(int i = 1; i <= 10; i++) {
        for(int j = 1; j <= 9; j++) {
    ...
    // Backtracking to find the local alignment path
    ...
    printf("\n--- METHOD 2.2: TOP-DOWN MEMOIZATION (LOCAL ALIGNMENT) ---\n");
    printf("Max Score: %d\n", max_s);
