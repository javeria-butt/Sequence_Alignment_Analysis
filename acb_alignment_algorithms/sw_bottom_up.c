...
void smith_waterman(char* s1, char* s2) {
    ...
    // 1. Matrix Filling (Bottom-Up Tabulation)
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            ...
            // Smith-Waterman characteristic: Score cannot be less than 0
            int res = 0;
            ...
            // Track the maximum score in the entire matrix
            if (dp[i][j] > max_score) {
                max_score = dp[i][j];
                max_i = i; max_j = j;
            }
    ...
    // 2. Backtracking from the Max Score position
    ...
    // 3. Output clearly labeled for your report
    printf("\n--- METHOD 2.3: SMITH-WATERMAN BOTTOM-UP (LOCAL ALIGNMENT) ---\n");
    printf("Max Score: %d\n", max_score);
...
int main() {
    // Your 10bp vs 9bp sequences
    smith_waterman("GACCTACAAG", "GACGTAGAA");
    return 0;
}
