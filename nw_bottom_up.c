...
void bottom_up(char* s1, char* s2) {
    ...
    // Printing Scoring Matrix
    printf("\n--- SCORING MATRIX (TABULATION) ---\n      ");
    for (int j = 0; j < m; j++) printf("%4c", s2[j]);
    printf("\n");
    for (int i = 0; i <= n; i++) {
        if(i > 0) printf("%c ", s1[i-1]); else printf("  ");
        for (int j = 0; j <= m; j++) {
            printf("%4d", dp[i][j]);
        }
        printf("\n");
    ...
    printf("\n--- METHOD 1.3: BOTTOM-UP (TABULATION) ---\n");
    printf("Score: %d\n", dp[n][m]);
...
int main() {
    bottom_up("GACCTACAAG", "GACGTAGAA");
    return 0;
}
