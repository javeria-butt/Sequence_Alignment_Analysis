...
void print_rec_sw(char* s1, char* s2, int i, int j, char* a, char* m, char* b, int* p) {
    ...
    if (cur == rec_sw(s1, s2, i - 1, j - 1) + score) {
...
int main() {
    char *s1 = "GACCTACAAG"; // Length 10
    char *s2 = "GACGTAGAA";  // Length 9
    int max_s = 0, mi = 0, mj = 0;

    // Searching all possible ending points for the best local match
    for(int i = 1; i <= 10; i++) {
        for(int j = 1; j <= 9; j++) {
    ...
    printf("\n--- METHOD 2.1: NAIVE RECURSIVE (LOCAL ALIGNMENT) ---\n");
    printf("Max Score: %d\n", max_s);
